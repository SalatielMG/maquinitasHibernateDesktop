...
CREATE DEFINER=`montezco`@`localhost` PROCEDURE `DevolucionesCerrados` (IN `idCaja` INT, IN `tipoDev` TINYINT)  BEGIN
...
CREATE DEFINER=`montezco`@`localhost` PROCEDURE `GastosCerrados` (IN `idCaja` INT)  BEGIN
...
CREATE ALGORITHM=UNDEFINED DEFINER=`montezco`@`localhost` SQL SECURITY DEFINER VIEW `vistacajaopen`  AS  select `caja`.`id` AS `id`,`caja`.`monto1` AS `monto1`,`caja`.`monto5` AS `monto5`,`caja`.`fecha` AS `fecha`,`caja`.`corte` AS `corte`,`caja`.`status` AS `status` from `caja` where ((`caja`.`corte` = 0) and (`caja`.`status` = 1)) ;
